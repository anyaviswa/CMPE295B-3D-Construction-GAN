        ...
        if self.model_idx % self.config.TEST.infer_freq == 0:
            ...
            if self.config.TEST.mode == "default":
                ...
                plot_path = os.path.join(
                    ...
                    # "%s.svg" % self.model_idx,
                    "%s.pdf" % self.model_idx,
                )
                ...
                print(type(self.ptcloud.squeeze().cpu().numpy()))
                print(self.ptcloud.squeeze().cpu().numpy())
                ...
                # 02958343 - car folder
                with open(benchmark_folder+'/'+str(self.model_id)+'.txt','w') as wf:
                  for x in self.ptcloud.squeeze().cpu().numpy():
                    wf.write(','.join([str(i) for i in x]))
                    wf.write('\n')
                ...
                #     "CD %.4f  EMD %.4f F-score %.4f Name: %s"
                #     % (self.metrics[1], self.metrics[2], self.metrics[0], self.model_id),
                #     [5, 0.5, 0.5],
                # )
                uv.plot_pcd_three_views(
                    plot_path,
